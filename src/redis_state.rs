    ...
    pub fn new(keys: Vec<RedisKey>) -> Self {
        let state = keys
            ...
            .fold(Value::Object(serde_json::Map::new()), |mut json, key| {
                if let InitialType::Initial(init) = &key.initial {
                    let _ = RedisState::update_key(&mut json, &key.key, init.clone());
                }
                json
            });
...
impl RedisKey {
    pub fn new(key: &str, key_type: KeyType, initial: InitialType) -> Self {
        RedisKey {
            key: key.to_string(),
            key_type,
            initial,
...
mod redis_tests {
    ...
    #[test]
    fn hacking() {
        let k1 = RedisKey::new("a/b/c", KeyType::Key, InitialType::Initial(json!("hej")));
        let k2 = RedisKey::new("a/b/d", KeyType::Key, InitialType::Initial(json!(1)));
        let k3 = RedisKey::new("b/c", KeyType::Map("nej".to_string()), InitialType::Initial(json!(true)));
    ...
    fn inserting() {
        let mut state = RedisState::new(vec![
            RedisKey::new("a/b/a", KeyType::Key, InitialType::Initial(json!(1))),
            RedisKey::new("a/b/b", KeyType::Key, InitialType::Initial(json!(false))),
            RedisKey::new("a/a/a", KeyType::Key, InitialType::Initial(json!("hej"))),
            RedisKey::new("a/a/b", KeyType::Key, InitialType::Initial(json!(false))),
            RedisKey::new("a/c/a", KeyType::Key, InitialType::Initial(json!(1))),
        ]);
